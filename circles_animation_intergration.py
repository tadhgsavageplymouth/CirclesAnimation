...
# Normalize the viewCount for visualization
df['viewCount'] = df['viewCount'] / df['viewCount'].max() * 10  # Scale viewCount to a max of 10 for circle size
...
for index, row in df.iterrows():
    ...
    circle = Circle((np.random.uniform(-10, 10), np.random.uniform(-10, 10)),
                    radius=0.01, edgecolor='black', facecolor='lightblue', alpha=0.7)
...
def update(frame):
    for i, circle in enumerate(circles):
        # Circles grow over time with a rate inversely proportional to video length
